...
\abstract{\textbf{This study introduces a set of features designed to capture
the motor symptoms of Parkinson’s Disease (PD) through detailed motion
analysis, with a specific focus on how these symptoms change before
and after medication. The features reflect key aspects of patient
movement, such as tremor intensity, slowness, rigidity, instability,
and irregularity in motion patterns. By quantifying how consistently
and smoothly a patient performs specific tasks, the features provide
insight into motor control quality and neurological function. Crucially,
they are labeled according to the patient's state---before and after
receiving medication---allowing for a clear comparison of treatment
effects. This enables not only objective tracking of symptom severity,
but also evaluation of medication responsiveness. These features address
...
scoring system used to determine the best biomarkers. Finally, Section
\ref{sec:Conclusions} discusses implications of the research results,
points out limitations of the study, and suggests directions of future
research.
...
\par\end{centering}
\caption{Resting Tremor Observation\label{fig:exercise0}}
...
\par\end{centering}
\caption{Test for Postural Tremor and Coordination\label{fig:exercise1}}
...
\par\end{centering}
\caption{Finger Tapping Speed Test\label{fig:exercise2}}
...
\par\end{centering}
\caption{Hand Opening-Closing Test\label{fig:exercise3}}
...
\item \textbf{Model-Based Importance} (\textbf{Random Fores}t): The Random
Forest classifier was built such that it could distinguish between
the two different states. The importance of a specific feature was
computed using the average reduction in Gini impurity across all trees
in the ensemble. Gini impurity is a measure of the purity of a node,
and the importance is given by the weighted cumulative reduction in
node impurity, divided by the probability of the node, and averaged
across trees \citep{patel}. The evaluation $\mbox{score}_{RF}$ for
a feature, f is given by:
\begin{equation}
\mbox{score}_{RF}=\frac{1}{N_{\mbox{trees}}}\sum\label{eq:rf}
\end{equation}
where $N_{\mbox{trees}}$ is the overall number of trees, $T$ is
one tree, and $\Delta\mbox{Gini}\left(n,f\right)$ decrease in Gini
impurity at node $n$ when a split is made along feature $f$. High-scoring
variables have a larger effect on the model predicting\textbf{ (Breiman,
2001).}
...
Component Analysis (PCA) model \textbf{(Jolliffe \& Cadima, 2016).}
\end{itemize}
...
\par\end{centering}
\caption{Flowchart of the used genetic algorithm.\label{fig:Flowchart-of-the}}
...
experiments.\label{fig:The-average-classification}}
