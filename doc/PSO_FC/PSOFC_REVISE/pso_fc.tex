...
\item Another interesting problem, which has been tackled by dozens of researchers
in the last decades, is that of overfitting of neural networks or
machine learning models in general. In this problem, although the
machine learning model has achieved a satisfactory level of training
this is not reflected in unknown patterns (test set ) that were not
present during training. The paper of Geman et al \citep{nngeman}
...
methods that reduce the number of parameters of the model ( pruning
methods ) \citep{nnprunning1,nnprunning2}, weight elimination \citep{nnelim1,nnelim2,nnelim3},
weight decaying methods \citep{nndecay1,nndecay2}, dropout methods
\citep{nndrop1,nndrop2}, the Sarprop method \citep{sarprop},\textbf{
}positive correlation methods \citep{nnpositive} etc. Recently, a
variety of paper have been proposed to handle the overfitting problem
in various cases such as usage of genetic algorithms for training
data selection in RBF networks\citep{rbfTrain}, the evolution of
RBF models using genetic algorithms for rainfall prediction \citep{rbfEvol},
pruning decision trees using genetic algorithms \citep{treePrun1,treePrun2}
etc.
\end{itemize}
...
(PSO) method \citep{pso_major,pso1,pso2}. The PSO method has been
selected as the optimization method due to its simplicity and the
small number of parameters that should be set. Also, the PSO method
...
\subsection{The used PSO method\label{subsec:The-used-PSO}}
...
was also found in the publication of Charilogis and Tsoulos \citep{ipso}.
...
where the number of particles in PSO algorithm increases from 100
...
method. The number of PSO particles increases from 100 to 400 and
...
\bibitem{nndecay2}I.G. TSoulos, A. Tzallas, D. Tsalikakis, Evolutionary
...
\bibitem{rbfTrain}C.R. Reeves, D.R. Bush, Using Genetic Algorithms
for Training Data Selection in RBF Networks. In: Liu, H., Motoda,
H. (eds) Instance Selection and Construction for Data Mining. The
Springer International Series in Engineering and Computer Science,
vol 608. Springer, Boston, MA, 2001.

\bibitem{rbfEvol}J. Wu, J. Long, M. Liu, Evolving RBF neural networks
for rainfall prediction using hybrid particle swarm optimization and
genetic algorithm, Neurocomputing \textbf{148}, pp. 136-142, 2015.

\bibitem{treePrun1}J. Chen, X. Wang, J. Zhai, Pruning Decision Tree
Using Genetic Algorithms, In: 2009 International Conference on Artificial
Intelligence and Computational Intelligence, Shanghai, China, pp.
244-248, 2009.

\bibitem{treePrun2}M.M. Mijwil, R. A. Abttan, Utilizing the Genetic
Algorithm to Pruning the C4.5 Decision Tree Algorithm, Asian Journal
of Applied Sciences 9, pp. 45-52, 201. 
