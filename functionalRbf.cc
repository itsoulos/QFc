...
QJsonObject    done(Data &x)
{
  ...
  int   tp=0,fp=0,tn=0,fn=0;
    ...
    for(int i=0;i<testx.size();i++)
    {
        ...
        per=tempOut-testy[i];
	double foundClass=nearestClass(tempOut);
        classError+=fabs(testy[i]-nearestClass(tempOut))>1e-7;
        sum+=per * per;
	if(dclass.size()==2)
	{
		
		if(isone(foundClass) && isone(testy[i])) tp++;
		else
		if(isone(foundClass) && iszero(testy[i])) fp++;
		else
		if(iszero(foundClass) && iszero(testy[i])) tn++;
		else
		if(iszero(foundClass) && isone(testy[i])) fn++;
		else ;
    ...
    if(dclass.size()==2)
    {
	double recall=tp*1.0/(tp*1.0+fn*1.0);
	double precision=tp*1.0/(tp*1.0+fp*1.0);
	printf("Recall: %.5lf Precission: %.5lf \n",recall,precision);
    }
